...
Sentry.init({
  ...
  // introduced them. Vercel auto-injects VERCEL_GIT_COMMIT_SHA at build
  // time (server-side); we use the NEXT_PUBLIC_* variant so client + server
  // emit the same release tag (single source of truth). Falls back to 'dev'
  // for local runs. Must match release.name in withSentryConfig.
