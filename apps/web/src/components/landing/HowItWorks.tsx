...
export function HowItWorks() {
  return (
    <section id="how" className="relative bg-ink py-20 sm:py-32">
      {/* Subtle top divider */}
      <div
        aria-hidden="true"
          ...
          <ol className="grid gap-10 md:grid-cols-4 md:gap-6">
            {STEPS.map(({ n, Icon, title, desc }) => (
              <li key={n} className="relative flex flex-col items-center text-center md:items-start md:text-left">
                <div className="relative z-10 flex h-24 w-24 items-center justify-center rounded-full border border-gold/30 bg-ink shadow-[0_0_40px_-12px_rgba(184,146,61,0.45)]">
                ...
                <h3 className="mt-2 font-heading text-lg font-semibold text-cream">{title}</h3>
                <p className="mt-2 max-w-[18rem] text-sm leading-relaxed text-cream/65">{desc}</p>
              </li>
            ))}
          </ol>
        </div>
