...
export function PricingPreview() {
  return (
    <section id="pricing" className="relative bg-ink py-20 sm:py-32">
            ...
            <article
              ...
              className={[
                ...
                t.highlight
                  ...
                  : 'border-cream/5 hover:border-gold/30',
              ].join(' ')}
              style={!t.highlight ? { backgroundColor: 'rgba(20, 20, 26, 0.4)' } : undefined}
            >
              {t.badge && (
                <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full border border-gold/40 bg-ink px-3 py-1 font-mono text-[10px] uppercase tracking-[0.2em] text-gold">
                  {t.badge}
                </span>
              )}
              <h3 className="font-heading text-lg font-semibold text-cream">{t.name}</h3>
              <p className="mt-1 text-sm text-cream/60">{t.desc}</p>
              <div className="mt-5 flex items-baseline gap-1.5">
                <span className="font-heading text-4xl font-bold text-cream">{t.price}</span>
                <span className="text-sm text-cream/55">VND {t.cadence}</span>
              </div>
