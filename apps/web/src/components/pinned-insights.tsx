    ...
    >
      ...
      <div className="flex-1 space-y-3 overflow-y-auto p-3">
        ...
        {items.map((p) => (
          <article
            key={p.id}
            className="rounded-md border border-gold/15 bg-card/60 p-3 text-xs text-foreground/90"
          >
            {p.title && (
            ...
            {onUnpin && (
              <button
                ...
                className="mt-2 text-eyebrow uppercase text-foreground/40 hover:text-gold"
              >
                Bỏ ghim
              </button>
            )}
