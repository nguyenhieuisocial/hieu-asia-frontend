...
export async function signOut(): Promise<void> {
  const supabase = getSupabaseAuth();
  if (!supabase) {
    // Even with no client we should clear the SSR hint if it lingers from a
    // previous build/session.
    clearAuthMarkerCookie();
    return;
  }
  await supabase.auth.signOut();
  // Wave 44.2 — explicit clear. onAuthStateChange should also fire SIGNED_OUT
  // but we don't rely on listener ordering for the redirect that typically
  // follows sign-out.
  clearAuthMarkerCookie();
