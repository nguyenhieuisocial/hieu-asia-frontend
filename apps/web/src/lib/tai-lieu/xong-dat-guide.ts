/**
 * Phần chữ CỐ ĐỊNH của tài liệu tặng "Tuổi xông đất Tết <can chi> <năm>".
 *
 ...
 *
 * Bước 2 có nhắc can chi + chi của năm nên KHÔNG thể là hằng số cấp module:
 * năm mục tiêu lật vào mùng 1 Tết, phải dựng lúc render — xem `lib/xong-dat.ts`.
 */

import type { YearProfile } from '@/lib/sinh-con';
...
export function howToChoose(target: YearProfile): GuideStep[] {
  return [
    {
    ...
    },
    {
      ...
      body: `Tết ${target.year} là năm ${target.canChi}, chi năm là ${target.zodiac.ten}. Người có tuổi thuộc nhóm tam hợp hoặc lục hợp với chi ${target.zodiac.ten} được xem là thuận. Người có tuổi xung với chi năm, hoặc trùng đúng tuổi ${target.zodiac.ten} (dân gian gọi là phạm Thái Tuế), thì theo lệ thường để dành cho dịp khác.`,
    },
    {
