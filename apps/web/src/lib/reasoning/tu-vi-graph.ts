...
Không liệt kê quá nhiều sao chi tiết — tập trung vào ý nghĩa tổng hợp.`;
  ...
  try {
    ...
    // Empty completion (content-filter / length-0) → treat as a failed palace
    // (throw → caught below → analysis:null) so it never feeds blank text into
    // the synthesis instead of a real analysis.
    return {
      palaces: [
        {
          palace: palace.name,
          context,
          analysis: requireText(result, `analyze_palace.${palace.name}`),
          error: null,
        },
      ],
    };
...
async function crossReference(state: State) {
  ...
  const system = `Bạn là một Tử Vi gia. Tìm các kết nối — tensions (mâu thuẫn) và synergies (cộng hưởng) — giữa các cung trong lá số. CHỈ dùng những cung CÓ phân tích bên dưới; KHÔNG suy diễn nội dung cho cung không xuất hiện. Văn phong calm, không phán xét.`;

  const prompt = `## ${valid.length} phân tích cung (trên 12 cung)
${palaceSummaries}
...
async function synthesize(state: State) {
  ...
  const prompt = `## Phân tích ${valid.length} cung (trên 12 cung)
${palaceSummaries}

CHỈ tổng hợp từ các cung CÓ phân tích ở trên. KHÔNG suy diễn/bịa nội dung cho cung không xuất hiện.
...
KHÔNG dùng các cụm: "định mệnh", "chắc chắn", "phải", "không thể", "sẽ xảy ra".`;
  ...
  const firstResult = await reasoningGenerate({
    ...
    system,
    prompt,
    // ~3500 ≈ 500–800 từ tiếng Việt (token-dense): tránh finishReason='length'
    // cắt bản đọc giữa câu rồi vẫn trả như thành công.
    maxOutputTokens: 3500,
    label: 'synthesize',
  });
  incrementCost(state.runId, 'top', firstResult.usage, 'synthesize');

  // Empty completion → throw (route returns 502) instead of shipping a blank
  // reading as success and burning the user's slot.
  const firstText = requireText(firstResult, 'synthesize');
  ...
  return { synthesis: requireText(retryResult, 'synthesize.retry') };
}
