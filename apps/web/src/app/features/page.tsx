...
function FeatureBadge({ badge }: { badge: Exclude<Badge, null> }) {
  if (badge === 'beta') {
    return (
      ...
      <span className="inline-flex items-center rounded-full border border-gold/60 px-2 py-0.5 font-mono text-eyebrow uppercase text-gold-700 dark:text-gold">
        Beta
      </span>
    );
  }
  return (
    ...
    <span className="inline-flex items-center rounded-full border border-jade/60 px-2 py-0.5 font-mono text-eyebrow uppercase text-jade-300">
      Mới
    </span>
  );
}
