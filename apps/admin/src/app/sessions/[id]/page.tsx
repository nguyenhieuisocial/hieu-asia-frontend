...
// (server-only HogQL via /api/admin/users/:id/journey). Surfaces, for this
// session's user, where they came from + the steps they took, without opening
// PostHog. Anon users carry an `anon-<session_id>` distinct_id; the route
// handles both forms.
// ───────────────────────────────────────────────────────────────────────────
...
async function fetchUserJourney(userId: string): Promise<UserJourneyResp> {
  try {
...
// Friendly Vietnamese labels for the ~38 PostHog events. Unknown events fall
// back to the raw name so future events still render.
...
function UserJourneyPanel({ userId }: { userId: string }) {
      ...
      <CardContent className="space-y-5">
        {journey.isLoading ? (
          ...
          <p className="text-xs italic text-muted-foreground">
            Cần{' '}
            <code className="font-mono text-gold/70">POSTHOG_PERSONAL_API_KEY</code>{' '}
            để xem hành trình.
          </p>
        ) : (
          <>
            ...
            <div className="space-y-2">
              ...
              {hasSource ? (
                ...
                <p className="text-sm text-muted-foreground">
                  Đến trực tiếp (không có nguồn theo dõi).
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              ...
              {events.length === 0 ? (
                <p className="text-sm italic text-muted-foreground">
                  Chưa có dữ liệu hành trình cho người này. (PostHog đã kết nối — sẽ
                  hiện khi có sự kiện.)
                </p>
              ) : (
                  ...
                  {events.map((e, i) => {
                    ...
                    return (
                      <li key={`${e.timestamp}-${i}`} className="relative">
                        ...
                        {path && (
                          <div className="truncate font-mono text-[11px] text-muted-foreground" title={e.url ?? undefined}>
                            {path}
                          </div>
                        )}
                      </li>
...
export default function SessionDetailPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Nguồn & hành trình — per-user attribution + event timeline from
          PostHog. Only when the session carries a user_id (authed user_id or
          anon-<session_id>); anon-less sessions have nothing to query. */}
      {s.user_id && <UserJourneyPanel userId={s.user_id} />}
