 ...
 * non-admin surface.
 *
 * Auth (Wave 48 P2-B defense-in-depth): middleware.ts already gates
 * /api/admin/* via verifySession. We re-verify the session inside the handler
 * so that if the middleware matcher ever changes (e.g. excludes /api/admin/*
 * by mistake), this service-role lookup still cannot be hit by an
