/** Admin proxy → Worker POST /admin/ledger/close (close a period, snapshot TB).
 *  Bookkeeping write → owner+. */
...
export async function POST(req: NextRequest) {
  ...
  const incoming = (await req.json().catch(() => ({}))) as { period_end?: string };
  const body = JSON.stringify({ period_end: incoming.period_end, by: auth.session.email });
  try {
    const r = await fetch(`${GATEWAY}/admin/ledger/close`, {
      method: 'POST',
      cache: 'no-store',
