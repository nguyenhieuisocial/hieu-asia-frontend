/**
 * Admin proxy to Worker `GET /admin/customers/:id`.
 */
...
export async function GET(
  ...
  // Wave 60.62.T1.4 — defense-in-depth verifySession backfill (customer PII read → viewer+).
  ...
  try {
    const r = await fetch(`${GATEWAY}/admin/customers/${encodeURIComponent(id)}`, {
      cache: 'no-store',
