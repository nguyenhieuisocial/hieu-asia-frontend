...
/**
 * Wave 60.62.T1.2 — extracted from /affiliates/broadcast/page.tsx.
 *
 * Admin broadcast composer + history. Sends subject/body to all (or
 * active-only) affiliates via in_app/email/telegram. Preserved as a tab so
 * mass-comms stay inside the consolidated /affiliates surface.
 */
...
export function BroadcastTab() {
  ...
  async function submit() {
    ...
    if (
      !confirm(
        `Gửi broadcast tới affiliates ${target === 'all' ? '(tất cả)' : '(active only)'}?`,
      )
    ) {
      return;
    }
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        In-app notifications hiển thị ngay trong dashboard. Email/Telegram được queue và gửi
        qua hệ thống delivery riêng.
      </p>
